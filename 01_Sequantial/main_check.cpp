...
// Норма невязки
double normResidual(const vector<vector<double>>& r) {
    return sqrt(dot(r, r));
}
...
void conjugateGradient(vector<vector<double>>& w, const vector<vector<double>>& a, 
                        ...
                        vector<double>& residual_history, vector<double>& solution_change_history) {
    // Инициализация
    ...
    double residual_norm = normResidual(r);
    double solution_change = normDifference(w_old, w);
    residual_history.push_back(residual_norm);
    solution_change_history.push_back(solution_change);
    ...
    for (int k = 1; k < max_iter; k++) {
        ...
        residual_norm = normResidual(r);
        solution_change = normDifference(w_old, w);
        
        residual_history.push_back(residual_norm);
        solution_change_history.push_back(solution_change);

        if (k % LOG_INTERVAL == 0 || k <= 5) {
            cout << "Итерация " << k << ": невязка = " << residual_norm 
                 << ", изменение решения = " << solution_change << endl;
        }
        
        // Проверка изменения решения
        if (solution_change < delta) {
...
// Сохранение истории сходимости в файл
void saveConvergenceHistory(const vector<double>& residual_history, 
                           const vector<double>& solution_change_history,
                           const string& filename) {
    ofstream file(filename);
    file << "iteration residual solution_change" << endl;
    for (size_t i = 0; i < residual_history.size(); i++) {
        file << i << " " << residual_history[i] << " " << solution_change_history[i] << endl;
    }
    file.close();
}
...
int main() {
    ...
    // Векторы для истории сходимости
    vector<double> residual_history;
    vector<double> solution_change_history;
    ...
    conjugateGradient(w, a, b, F, residual_history, solution_change_history);
    ...
    saveConvergenceHistory(residual_history, solution_change_history, "convergence_history.txt");
    cout << "Диагностические данные сохранены в convergence_history.txt" << endl;
