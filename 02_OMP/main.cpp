...
int main(int argc, char* argv[]) {
    // Установка числа потоков по умолчанию
    ...
    if (argc > 1) {
        ...
        if (num_threads <= 0) {
            ...
            cout << "Неверное число потоков. Используется максимальное: " << num_threads << endl;
        }
    }
    ...
    cout << "Параметр delta = " << delta << endl;
    cout << "Используется потоков: " << num_threads << endl << endl;
    ...
    cout << "Общее время выполнения: " << omp_get_wtime() - start_time << " секунд" << endl;
